    ...
    <div className="grid grid-cols-1 gap-6 max-w-5xl">
      <div className="rounded-lg border border-slate-700/50 bg-slate-900/50 p-6">
        ...
        <p className="mb-5 text-sm text-slate-400">
          Wähle ein Theme mit globalem Farbverlauf. Es wird direkt auf alle
          Seiten angewendet.
        </p>
                ...
                >
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <span className="text-sm font-medium text-slate-200">
                      {themeLabels[tIter] ?? tIter}
                    </span>
                    {isActive && (
                      <span className="text-xs text-text">Aktiv</span>
                    )}
                  </div>
                </button>
              </Card>
        ...
        <div className="mt-5 rounded-lg border border-slate-700/50 bg-slate-800/40 p-4">
          ...
          <p className="mt-1 text-sm text-slate-200">
            {themeLabels[theme] ?? themeConfig.label}
          </p>
          ...
          <div className="mt-6 rounded-lg border border-slate-700/50 bg-slate-800/40 p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-5 gap-3 items-end">
              <div>
                ...
                <Input
                  ...
                  max={255}
                  value={String(rgb.r)}
                  onChange={(e) => setRgbChannel("r", e.target.value)}
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">
                ...
                <Input
                  ...
                  max={255}
                  value={String(rgb.g)}
                  onChange={(e) => setRgbChannel("g", e.target.value)}
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">
                ...
                <Input
                  ...
                  max={255}
                  value={String(rgb.b)}
                  onChange={(e) => setRgbChannel("b", e.target.value)}
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-slate-400 mb-1">
            ...
            <div className="mt-3 flex items-center gap-3">
              ...
              <p className="text-xs text-slate-400">
                rgb({rgb.r}, {rgb.g}, {rgb.b}) / {hex.toUpperCase()}
              </p>
            </div>
          </div>
        )}
