...
export default function MarkdownDocs() {
  ...
  const [relationFilter, setRelationFilter] = useState<
    "ALL" | "HARDWARE" | "SERVICE" | "UNASSIGNED"
  >("ALL");
    ...
    return topLevelDocs.filter((doc) => {
      const matchesSearch =
        ...
        String(doc.title || "")
          .toLowerCase()
          .includes(query) ||
        String(doc.hardwareAsset?.name || "")
          .toLowerCase()
          .includes(query) ||
        String(doc.softwareUnit?.name || "")
          .toLowerCase()
          .includes(query);
  ...
  const markdownComponents = {
    h1: ({ children }: any) => (
      <h1 className="text-lg font-bold text-text mt-3 mb-2">{children}</h1>
    ),
    h2: ({ children }: any) => (
      <h2 className="text-base font-bold text-text mt-3 mb-2">{children}</h2>
    ),
    h3: ({ children }: any) => (
      <h3 className="text-sm font-semibold text-text mt-2 mb-1">{children}</h3>
    ),
    p: ({ children }: any) => (
      <p className="text-sm text-slate-400 leading-relaxed mb-2">{children}</p>
    ),
    ul: ({ children }: any) => (
      <ul className="list-disc list-inside pl-2 space-y-1 text-sm text-slate-400">
        {children}
      </ul>
    ),
    ol: ({ children }: any) => (
      <ol className="list-decimal list-inside pl-2 space-y-1 text-sm text-slate-400">
        {children}
      </ol>
    ),
    ...
    code: ({ children }: any) => (
      <code className="px-1.5 py-0.5 rounded bg-slate-800 border border-slate-700/50 text-xs text-purple-400">
        {children}
      </code>
    ),
    a: ({ href, children }: any) => {
      ...
      return (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="text-purple-400 hover:text-purple-400/80 underline underline-offset-2"
        >
          {children}
        </a>
      );
  ...
  const deleteDoc = async () => {
    ...
    if (!window.confirm("Delete this markdown document and all child docs?"))
      return;
      ...
      <div className="h-full flex flex-col min-h-0">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-12 gap-6 page-content-scroll">
          <Card className="xl:col-span-4 rounded-xl border border-slate-700/50 bg-slate-900/50 p-0 overflow-hidden h-fit">
            <div className="px-4 py-3 border-b border-slate-700/50 bg-slate-800 text-sm font-semibold text-slate-400">
                ...
                >
                  ...
                  <Select.Popover className="w-[var(--trigger-width)]">
                    <ListBox>
                      ...
                      <ListBox.Item id="HARDWARE" className="pl-2">
                        Hardware-linked
                      </ListBox.Item>
                      <ListBox.Item id="SERVICE" className="pl-2">
                        Service-linked
                      </ListBox.Item>
            ...
            <div className="divide-y divide-border">
              {filteredTopLevelDocs.length === 0 && (
                <p className="p-4 text-slate-400">
                  No top-level markdown files found.
                </p>
              )}
              {filteredTopLevelDocs.map((doc) => {
                ...
                const docTags = [
                  ...
                  doc.hardwareAsset?.name
                    ? `Hardware: ${doc.hardwareAsset.name}`
                    : null,
                  childCount > 0
                    ? `${childCount} ${childCount === 1 ? "child" : "children"}`
                    : null,
                  !doc.hardwareAssetId && !doc.softwareUnitId
                    ? "Unassigned"
                    : null,
                ...
                return (
                  <Button
                    ...
                    className={`w-full !h-auto !min-h-0 text-left px-4 py-2.5 transition-all !border-0 !ring-0 !shadow-none ${
                      ...
                      {docTags.length > 0 && (
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {docTags.map((tag) => (
                            <span
                              key={`${doc.id}-${tag}`}
                              className="text-[11px] border border-slate-700/60 bg-slate-800/70 text-slate-300 px-2 py-0.5 rounded-full"
                            >
                              {tag}
                            </span>
                          ))}
          ...
          <Card className="xl:col-span-8 rounded-xl border border-slate-700/50 bg-slate-900/50 p-6">
            {!selectedDoc && (
              <p className="text-slate-400">
                Select a top-level markdown file on the left.
              </p>
            )}
              ...
              <div className="space-y-4">
                <div>
                  <div className="flex items-start justify-between gap-3">
                    <h3 className="text-xl font-bold text-text break-words">
                      {selectedDoc.title}
                    </h3>
                    <Button
                      type="button"
                  ...
                  <div className="flex gap-2 mt-2 flex-wrap">
                    {selectedDoc.hardwareAsset?.name && (
                      <span className="text-xs bg-purple-600/15 text-purple-400 px-2 py-1 rounded-full">
                        HW: {selectedDoc.hardwareAsset.name}
                      </span>
                    )}
                    {selectedDoc.softwareUnit?.name && (
                      <span className="text-xs bg-blue-500/15 text-blue-300 px-2 py-1 rounded-full">
                        Service: {selectedDoc.softwareUnit.name}
                      </span>
                    )}
                    {selectedDoc.parentDoc?.title && (
                      <span className="text-xs bg-purple-500/15 text-purple-300 px-2 py-1 rounded-full">
                        Parent: {selectedDoc.parentDoc.title}
                      </span>
                    )}
                    {!selectedDoc.hardwareAssetId &&
                      !selectedDoc.softwareUnitId &&
                      !selectedDoc.parentDocId && (
                        <span className="text-xs bg-slate-800 border border-slate-700/50 text-slate-400 px-2 py-1 rounded-full">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedDocChildren.map((child) => (
                        <Button
                          ...
                          className={`text-xs px-3 py-1.5 rounded-full border transition-colors !ring-0 !shadow-none ${
                            selectedDocId === child.id
