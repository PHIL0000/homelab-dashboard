...
export default function DocumentationMap() {
  ...
  <Card className="rounded-xl border border-slate-700/50 bg-slate-900/50 p-6 mb-6">
        <h3 className="text-lg font-semibold text-text mb-4">Hardware → Services Tree</h3>
        <div className="space-y-4">
          {hardware.length === 0 && <p className="text-slate-400">No hardware available.</p>}

          {hardware.map(hw => {
            const deps = deploymentByHardware.get(hw.id) || [];
            return (
              <div key={hw.id} className="border border-slate-700/50 rounded-lg p-4 bg-slate-800/40">
                <div className="font-semibold text-purple-400">🖥 {hw.name}</div>
                <div className="text-xs text-slate-400 mt-1">{hw.type || 'OTHER'} • {hw.ip || 'no ip'} • {hw.status || 'UNKNOWN'}</div>

                <div className="mt-3 pl-4 border-l border-slate-700/50 space-y-2">
                  {deps.length === 0 && <p className="text-sm text-slate-400">└─ No deployed services</p>}

                  {deps.map(dep => (
                    <div key={dep.id} className="text-sm">
                      <p className="text-slate-100">└─ ⚙️ {dep.softwareUnit?.name || 'Unknown service'}</p>
                      <p className="ml-6 text-xs text-slate-400">{dep.softwareUnit?.type || 'OTHER'} • {dep.internalIp || 'no internal ip'} • {dep.status || 'UNKNOWN'}</p>
                    </div>
                  ))}
                </div>
              </div>
  ...
  <Card className="rounded-xl border border-slate-700/50 bg-slate-900/50 p-6">
        ...
        {unassignedServices.length === 0 ? (
          <p className="text-slate-400">All services are currently assigned to hardware.</p>
        ) : (
          <ul className="space-y-2">
            {unassignedServices.map(sw => (
              <li key={sw.id} className="text-sm text-slate-100">
                ⚙️ {sw.name} <span className="text-slate-400">({sw.type || 'OTHER'})</span>
              </li>
            ))}
          </ul>
        )}
