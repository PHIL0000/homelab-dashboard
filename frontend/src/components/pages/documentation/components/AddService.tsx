...
export default function AddService({
  ...
  const addStorageId = (storageId: string) => {
    ...
    onSelectedStorageIdsChange(
      selectedIds.includes(storageId) ? selectedIds : [...selectedIds, storageId],
    );
    setStoragePickerKey(STORAGE_PICKER_NONE_KEY);
  };
            ...
            <form onSubmit={onSubmit} className="p-5 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <label className="space-y-1">
                  ...
                  <Select
                    selectedKey={type}
                    onChange={(key) => {
                      if (key != null) onTypeChange(String(key));
                    }}
                    className="w-full"
                  >
                    <Select.Trigger className="w-full px-3 flex items-center justify-between">
              ...
              {showInternalIp && (
                <label className="space-y-1 block">
                  ...
                  <Input
                    value={internalIp || ""}
                    onChange={(e) => onInternalIpChange?.(e.target.value)}
                    className="w-full"
                  />
                  <p className="text-[11px] text-slate-400">
                    IP address where this service is reachable. Usually the hardware IP,
              ...
              {!!storageOptions?.length && (
                <div className="space-y-1 block">
                  ...
                  >
                    ...
                    <Select.Popover className="w-[var(--trigger-width)]">
                      <ListBox>
                        <ListBox.Item id={STORAGE_PICKER_NONE_KEY} className="pl-2">
                          Select storage to add
                        </ListBox.Item>
                    ...
                    {selectedIds.length === 0 ? (
                      ...
                      <div className="flex flex-wrap gap-2">
                        {selectedIds.map((storageId) => {
                          const label =
                            storageOptions.find((option) => option.id === storageId)?.name ||
                            storageId;
                          return (
                            <span
                              key={storageId}
