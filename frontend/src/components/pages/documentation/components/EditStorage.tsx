  ...
  useEffect(() => {
    ...
    const assignmentIds = Array.isArray(storage.serviceAssignments)
      ? storage.serviceAssignments
          .map((assignment: any) =>
            String(assignment.softwareUnitId || assignment.softwareUnit?.id || ""),
          )
          .filter(Boolean)
      : [];
    ...
    if (
      storage.usableSpaceGB &&
      storage.usableSpaceGB >= 1000 &&
      storage.usableSpaceGB % 1000 === 0
    ) {
                  ...
                  <label className="space-y-1">
                    ...
                    <Select
                      ...
                      onChange={(key) => {
                        if (
                          key != null &&
                          (String(key) === "GB" || String(key) === "TB")
                        )
                          setSpaceUnit(String(key) as "GB" | "TB");
                      }}
                      className="w-full"
                    >
                      <Select.Trigger className="w-full px-3 flex items-center justify-between">
